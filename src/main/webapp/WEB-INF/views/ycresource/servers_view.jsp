                            ...
                            <table id="ycresource_tb" class="table table-bordered table-striped">
                                ...
                                <c:forEach items="${ycresources}" var="ycresource">
                                    <tr>
                                        ...
                                        <td>${ycresource.type}</td>
                                        <td>${ycresource.ostype}</td>
                                        <td>${ycresource.ip}</td>
                                        <td>${ycresource.username}</td>
                                        <td>${ycresource.password}</td>
                                        <td>${ycresource.usedesc}</td>
                ...
                <form role="form" id="add_form">
                    <div class="box-body">
                        <div class="form-group" id="fileuploadDiv">
                            <label>服务器名称</label>
                            <input type="text" name="name" id="fileuploadName" class="form-control" placeholder="服务器名称（必填）">
                        </div>
                        <div class="form-group">
                            <label>服务器类型</label>
                            <input type="text" name="type" class="form-control" placeholder="操作系统类型（必填）">
                        </div>
                        <div class="form-group">
                            <label>操作系统类型</label>
                            <input type="text" name="ostype" class="form-control" placeholder="操作系统类型（必填）">
                        </div>
                        <div class="form-group">
                            <label>ip地址</label>
                            <input type="text" name="ip" class="form-control" placeholder="ip地址（必填）">
                        </div>
                        <div class="form-group">
                            <label>用户名</label>
                            <input type="text" name="username" class="form-control" placeholder="用户名（选填）">
                        </div>
                        <div class="form-group">
                            <label>密码</label>
                            <input type="text" name="password" class="form-control" placeholder="密码（选填）">
                        </div>
                        <div class="form-group">
                            <label>用处描述</label>
                            <input type="text" name="usedesc" class="form-control" placeholder="用处描述（选填）">
                        </div>
                        <div class="form-group">
                            <label>是否可用</label>
                            <br>
            ...
            <div class="modal-body">
                <form role="form" id="update_form">
                    <div class="box-body">
                        <div class="form-group">
                            <label>编号</label>
                            <input type="text" name="id" class="form-control" placeholder="编号（必填）" readonly>
                        </div>
                        <div class="form-group">
                            <label>服务器名称</label>
                            <input type="text" name="name" id="fileuploadName_update" class="form-control" placeholder="服务器名称（必填）">
                        </div>
                        <div class="form-group">
                            <label>服务器类型</label>
                            <input type="text" name="type" class="form-control" placeholder="服务器类型（必填）">
                        </div>
                        <div class="form-group">
                            <label>操作系统类型</label>
                            <input type="text" name="ostype" class="form-control" placeholder="操作系统类型（必填）">
                        ...
                        <div class="form-group">
                            <label>密码</label>
                            <input type="text" name="password" class="form-control" placeholder="密码（必填）">
                        </div>
                        <div class="form-group">
                            <label>用处描述</label>
                            <input type="text" name="usedesc" class="form-control" placeholder="用处描述（必填）">
                        </div>
                        <div class="form-group">
                            <label>是否可用</label>
                            <br>
...
<script>
    $(function () {
        ...
        $("#add_submit").click(function () {
            ...
            $.ajax({
                type: "POST",
                url: "ycresourceserver/server-create",
                ...
                success: function (data) {
                    if (data.meta.success) {
                        ...
                        window.location = "ycresourceserver/servers-view.html";
                    } else {
        ...
        $("#update_submit").click(function () {
            ...
            $.ajax({
                type: "POST",
                url: "ycresourceserver/" + data.id + "/update",
                data: data,
                dataType: "json",
                success: function (data) {
                    if (data.meta.success) {
        ...
        $("#del_submit").click(function () {
            ...
            $.ajax({
                type: "GET",
                url: "ycresourceserver/" + id + "/delete",
                success: function (data) {
                    console.log(data);
                    if (data.meta.success) {
                        //添加成功
    ...
    function buildUpdateModal(obj) {
        ...
        console.log(tds[2]);
        if (tds[7].innerHTML == 1) {
            //禁用
            console.log("forbidden");
            $(inputs[8]).prop('checked', 'checked');

        } else {
            console.log("opened");
            $(inputs[9]).prop('checked', 'checked');
        }
        $('input[type="checkbox"].flat-red, input[type="radio"].flat-red').iCheck({
            checkboxClass: 'icheckbox_flat-green',
            radioClass: 'iradio_flat-green'
