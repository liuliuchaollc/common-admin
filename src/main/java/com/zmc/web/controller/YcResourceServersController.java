...
@Controller
@RequestMapping("ycresourceserver")
public class YcResourceServersController {
    @Autowired
    private YcResourceServersService ycResourceService;
     ...
     * @throws Exception
     */
    @RequestMapping(value = "/servers-view.html",method = RequestMethod.GET)
    public String ycResourceView(Model model) throws Exception {
        List<YcResourceServer> roles = ycResourceService.findAllYcResources();
        model.addAttribute("ycresources",roles);
        return "ycresource/servers_view";
    }
    ...
    public  ResponseEntity<byte[]> downloadFile2(@PathVariable String ycresourceid) {  
        ...
        try{
	    	YcResourceServer ycr = ycResourceService.findYcResourceById(Long.parseLong(ycresourceid));
    ...
    @ResponseBody
    public Response ycResourceAdd(YcResourceServer server, @CurrentUser User user){
        Response response = new Response();
        if (StringUtils.isEmpty(server.getName())){
            return  response.failure("角色名称不能为空");
        }
        try {
        	server.setCreate_by(user.getUsername());
            Date time = new Date();
            server.setCreate_time(time);
            server.setUpdate_time(time);
            server.setUpdate_by(user.getUsername());
            ycResourceService.addYcResource(server);
    ...
    @ResponseBody
    public Response ycResourceUpdate(@PathVariable String id,YcResourceServer server){
        Response response = new Response();
        if (StringUtils.isEmpty(server.getName())){
            return response.failure("角色名称不能为空");
        }
        ...
        try {
        	...
        	server.setUpdate_by((String) SecurityUtils.getSubject().getPrincipal());
            Boolean result = ycResourceService.updateYcResource(server);
            if (result){
                return response.success(server);
            }else {
